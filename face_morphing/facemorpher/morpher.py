...
def morpher(name, imgpaths, width=500, height=600, num_frames=20, fps=10,
  ...
  for dest_img, dest_points in images_points_gen:
    morph(name, src_img, src_points, dest_img, dest_points,
          width, height, num_frames, fps, out_frames, plot, background)
...
def main():
  ...
  verify_args(args)
  name = args['--src'][-11:-4]+"_"+args['--dest'][-11:-4]+".png"
  morpher(name, list_imgpaths(args['--images'], args['--src'], args['--dest']),
