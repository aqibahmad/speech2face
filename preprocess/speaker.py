...
class Speaker():
	...
	def __init__(self, sample_rate = 16000, duration = 6, mono = True, window = 400, stride = 160, fft_length = 512, amp_norm = 0.3, verbose = False):
		self.videos_path = "./preprocess/data/videos/"
		self.audios_path = "./preprocess/data/audios/"
		self.spect_path  = "./preprocess/data/audio_spectrograms/"
	...
	def extract_wav(self, filename):
		...
		if (not os.path.isfile(self.spect_path + filename  + ".pkl")):
			if (not os.path.isfile(self.audios_path + wavfile)):
				#ffmpeg -nostats -loglevel 0 -t 
				os.popen("ffmpeg -t " + str(self.duration) + " -stream_loop -1  -i " + self.audios_path + filename + ".m4a" + " -vn " + self.spect_path + wavfile).read()
